...
/// `crates/gar-cli/tests/tui.rs` and `tests/persist.rs` can drive
...
/// Entry point shared by the `gar` binary and any library consumer.
///
/// # Errors
///
...
fn run_view(view: &ViewArgs) -> Result<()> {
    ...
    if view.file.is_none() && !view.interactive {
        ...
        let lens = cli::build_lens(view.lens, view.time_scale, view.purist, false);

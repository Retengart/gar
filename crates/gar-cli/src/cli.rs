...
impl LensMode {
    ...
    /// `gar::LensMode` so integration tests under `tests/` can iterate
...
///
/// When `tui` is `true`, the cuneiform lens forces ASCII fallback because
/// Cuneiform Unicode characters have ambiguous terminal width — many
...
pub(crate) fn build_lens(mode: LensMode, scale: TimeScale, purist: bool, tui: bool) -> Option<Box<dyn Lens>> {
    match mode {
        ...
        LensMode::Cuneiform => Some(Box::new(CuneiformLens {
            fallback: tui || cuneiform::ascii_fallback_forced(),
        })),
    }
}
...
/// `gar::__TuiTimeScale` can forward it to integration tests that
/// drive `gar::__test_hooks::run_with_terminal`. No stability
...
pub(crate) enum Command {
    ...
    /// raw bytes on stdout.
    ///
    /// Useful for roundtripping — `gar --color=never FILE | gar
...
/// [`Cli`] so `gar FILE` keeps its current shape.
#[derive(Args, Debug)]
...
mod tests {
    ...
    fn all_methods_total_over_all() {
        for &mode in LensMode::ALL {
            ...
            let _lens = build_lens(mode, TimeScale::default(), false, false);

...
//!
//! ```text
//! gar --color=never FILE | gar decode > FILE.roundtrip
//! ```
//!
...
//! # Length metadata
//!
//! Dumps emitted by `gar` v2+ carry a trailing `# bytes=0x<hex>`
    ...
    if trailer.is_none() && any_chunk_seen {
        eprintln!(
            "decode: no length metadata; assuming input was 8-byte-aligned. \
             ...
             gar v2+ to silence this warning.",
        );
    }
